...
class CropAttriMappingDatasetBin(Dataset):
    ...
    def _preload_bin_paths(self):
        ...
        self.shard_sample_counts = np.array(self.shard_sample_counts, dtype=np.int64)
        self.shard_ends = np.cumsum(self.shard_sample_counts)
        self.n_samples = int(self.shard_ends[-1])
        print(f"Preloaded {len(self.bin_paths)} binary file(s) with total {self.n_samples} samples")

    def _init_memmap(self):
        """初始化memmap映射（按需打开分片）"""
        self._memmaps = [None] * len(self.bin_paths)

    def _ensure_memmap(self, shard_idx):
        mm = self._memmaps[shard_idx]
        if mm is None:
            mm = np.memmap(self.bin_paths[shard_idx], dtype=self.dtype, mode='r')
            total_floats = mm.size
            if total_floats % self.floats_count != 0:
                raise ValueError(f"浮点数量 {total_floats} 不是每样本浮点数 {self.floats_count} 的整数倍: {self.bin_paths[shard_idx]}")
